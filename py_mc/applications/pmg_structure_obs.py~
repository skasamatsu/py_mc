...
from py_mc.applications.rhorhocy import rho_autocorr, rho_autocorr_fft
...
def isotropic_gauss_nd(x,xcenter,sigma):
    return np.prod(1./(np.sqrt(2.*np.pi*sigma))*np.exp(-np.power((x-xcenter)/sigma, 2.)/2.))

def gauss_1d(x,center,sigma):
    return 1./(np.sqrt(2.*np.pi)*sigma)*np.exp(-np.power((x-xcenter)/sigma, 2.)/2.)
...
def rho_gauss(x, structure, species, sigma, comm=None):
    ...
    if comm:
        ...
        rho[rank*numgrid_loc:(rank+1)*numgrid_loc] = \
            np.sum(np.exp(-np.power(dist/sigma,2.)/2.), axis=1)
        ...
        rho = np.sum(np.exp(-np.power(dist/sigma,2.)/2.), axis=1)
        
    rho /= (np.sqrt(2.*np.pi)*sigma)
    return rho
...
def write_rho_vasp(structure, species, sigma, mesh, filename):
    ...
    frac_coor_grid = np.array(
        np.meshgrid(range(mesh[0]),range(mesh[1]),range(mesh[2]),
                    indexing="ij")).T.reshape(-1,3)/np.array(mesh)
