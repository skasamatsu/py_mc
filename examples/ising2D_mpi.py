...
if __name__ == "__main__":
    ...
    #parallelCalc.reload()
    obs_mean = parallelCalc.run(nsteps, RXtrial_frequency, sample_frequency, observer=observer())

    if comm.Get_rank()==0:
        for i in range(len(kTs)):
            print(kTs[i], "\t", "\t".join([str(obs_mean[i][j]/nspin) for j in range(len(obs_mean[i]))]))
